package com.rodrigo.dummymigrationliquibase.util;
...
public class DatabaseCleaner {
    ...
    private void checkTestDatabase() throws SQLException {
        String catalog = connection.getCatalog().toLowerCase();

        if (catalog == null || !catalog.endsWith("test")) {
            throw new RuntimeException(
                    "Não é possível apagar os dados de '" + catalog + "', porque não é um banco de teste (sufixo 'test' não encontrado).");
        }
    }
